...
namespace SangoUtils_IOCP
{
    public class IOCPPeer<T> where T : IClientPeer_IOCP, new()
    {
        ...
        {
            IOCPLogger.SetLogger(IOCPRunnerType.ConsoleProject);
            OpenAsClient(ip, port);
        }
        public void OpenAsUnityClient(string ip, int port)
        {
            IOCPLogger.SetLogger(IOCPRunnerType.UnityProject);
            OpenAsClient(ip, port);
        }
        ...
        public void CloseAsServer()
        {
            if (_peerList != null)
            {
                for (int i = 0; i < _peerList.Count; i++)
                {
                    _peerList[i].OnClientClosed();
                }
                _peerList.Clear();
                _peerList = null;
            }
