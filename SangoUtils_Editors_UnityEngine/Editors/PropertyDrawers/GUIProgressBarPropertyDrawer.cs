...
{
    [CustomPropertyDrawer(typeof(GUIProgressBarAttribute))]
    public class GUIProgressBarPropertyDrawer : BaseGUIPropertyDrawer
    {
        protected override float GetPropertyHeight_Internal(SerializedProperty property, GUIContent label)
        {
            GUIProgressBarAttribute progressBarAttribute = GUIDrawerPropertyUtils.GetAttribute<GUIProgressBarAttribute>(property);
        ...
        protected override void OnGUI_Internal(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            if (maxValue != null && IsNumber(maxValue))
            {
                ...
                var indentLength = GUIDrawerInspectorEditorUtils.GetIndentLength(rect);
        ...
        private object GetMaxValue(SerializedProperty property, GUIProgressBarAttribute progressBarAttribute)
        {
            ...
            else
            {
                ...
                FieldInfo valuesFieldInfo = GUIDrawerReflectionUtils.GetField(target, progressBarAttribute.MaxValueName);
                if (valuesFieldInfo != null)
                {
                    return valuesFieldInfo.GetValue(target);
                }

                PropertyInfo valuesPropertyInfo = GUIDrawerReflectionUtils.GetProperty(target, progressBarAttribute.MaxValueName);
                if (valuesPropertyInfo != null)
                {
                    return valuesPropertyInfo.GetValue(target);
                }

                MethodInfo methodValuesInfo = GUIDrawerReflectionUtils.GetMethod(target, progressBarAttribute.MaxValueName);
