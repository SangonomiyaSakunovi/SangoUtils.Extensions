...
namespace SangoUtils.AOICells
{
    public class AOIController
    {
        ...
        private Dictionary<string, AOICell> _aoiCellDict = new Dictionary<string, AOICell>();
        private List<AOIEntity> _aoiEntities = new List<AOIEntity>();
        ...
        public AOIController(string controllerName)
        {
        ...
        public void OnEntityExitCell(AOIEntity entity)
        {
            ...
            {
                AOISession.Instance.LogErrorFunc?.Invoke($"AOICellDict can`t find EntityKey: [ {entity.AOICellKey} ].");
            }
            if (!_aoiEntities.Remove(entity))
            {
                AOISession.Instance.LogErrorFunc?.Invoke($"AOIEntityList can`t find EntityKey: [ {entity.AOICellKey} ].");
            }
        }
        ...
        public void OnEntityMoveInsideAOICell(AOIEntity entity)
        {
            ...
            {
                AOISession.Instance.LogWarningFunc?.Invoke($"AOICellDict can`t find AOICellKey: [ {entity.AOICellKey} ].");
            }
        }
        ...
        public AOICell GetOrNewAOICell(AOIEntity entity)
        {
            ...
            {
                AOICell newCell = new AOICell(entity.AOICellIndex, this);
        ...
        public void MarkExitEntityCell(AOIEntity entity)
        {
            ...
            {
                AOISession.Instance.LogWarningFunc?.Invoke($"AOICellDict can`t find AOICellKey when MarkExitEntityCell: [ {entity.AOICellKey} ].");
            }
        }
        ...
        private void GetAOIAroundCell(AOICell cell)
        {
            ...
            for (int i = xIndex - 2; i < xIndex + 3; i++)
            {
                for (int j = zIndex - 2; j < zIndex + 3; j++)
                {
                    ...
                    {
                        AOICellIndex newAOICellIndex = new AOICellIndex(i, j);
